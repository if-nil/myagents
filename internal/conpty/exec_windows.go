...
// Adapted (verbatim) from github.com/charmbracelet/x/conpty v0.1.1 (MIT
// License, Copyright (c) Charmbracelet, Inc), which in turn copies these
// helpers from the Go standard library.
...
func execEnvDefault(sys *syscall.SysProcAttr) (env []string, err error) {
	...
	// Walk the double-NUL-terminated block entry by entry. This deviates from
	// the upstream copy, which round-tripped the pointer through uintptr —
	// unsafe.Add/unsafe.Slice keep vet's unsafeptr check happy and the GC
	// informed.
	for p := block; ; {
		// find NUL terminator
		n := 0
		...
		env = append(env, string(utf16.Decode(unsafe.Slice(p, n))))
		// Skip past the entry and its NUL terminator (uint16s, hence *2).
